...
public class TransitionSettingsManager: ObservableObject {
    ...
    private let settingsKey = "PhotoSlideshowTransitionSettings"
    
    public init() {
        if let data = userDefaults.data(forKey: settingsKey),
           let savedSettings = try? JSONDecoder().decode(TransitionSettings.self, from: data) {
    ...
    public func resetToDefault() {
        ...
        // Notify observers of settings change
        NotificationCenter.default.post(name: .transitionSettingsChanged, object: settings)
    }
    
    public func applyPreset(_ preset: TransitionSettings) {
        settings = preset
        saveSettings()
    ...
    /// Toggle transition effects on/off
