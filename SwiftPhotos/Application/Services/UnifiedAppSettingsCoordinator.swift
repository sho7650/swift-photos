...
/// Unified central coordinator for all application settings
/// Uses the unified settings management system to eliminate code duplication
@Observable
@MainActor
public final class UnifiedAppSettingsCoordinator {
    
    // MARK: - Unified Settings Managers
    public let performance: UnifiedPerformanceSettingsManager
    public let slideshow: UnifiedSettingsManager<SlideshowSettings>
    public let sort: UnifiedSortSettingsManager
    public let transition: UnifiedSettingsManager<TransitionSettings>
    public let uiControl: UnifiedUIControlSettingsManager
    ...
    public init() {
        self.performance = UnifiedPerformanceSettingsManager()
        self.slideshow = UnifiedSettingsManager<SlideshowSettings>()
        self.sort = UnifiedSortSettingsManager()
        self.transition = UnifiedSettingsManager<TransitionSettings>()
        self.uiControl = UnifiedUIControlSettingsManager()
        
        ProductionLogger.lifecycle("UnifiedAppSettingsCoordinator initialized with unified settings managers")
    ...
    public func exportAllSettings() throws -> Data {
        ...
        let exportData = UnifiedSettingsExportData(
    ...
    public func importAllSettings(from data: Data, merge: Bool = false) throws {
        ...
        let importData = try JSONDecoder().decode(UnifiedSettingsExportData.self, from: data)
        ...
        if !merge {
            ...
            try mergeImportedSettings(importData)
        }
    ...
    /// Validate all settings and fix inconsistencies
    public func validateAllSettings() -> [String] {
        var issues: [String] = []
        
        // Validate individual settings
        if !performance.validateSettings() {
            issues.append("Performance settings validation failed")
        }
        
        if !slideshow.validateSettings() {
            issues.append("Slideshow settings validation failed")
        }
        
        if !sort.validateSettings() {
            issues.append("Sort settings validation failed")
        }
        
        if !transition.validateSettings() {
            issues.append("Transition settings validation failed")
        }
        
        if !uiControl.validateSettings() {
            issues.append("UI Control settings validation failed")
        }
        
        // Check cross-setting consistency
        let consistencyIssues = validateAndFixConsistency()
        issues.append(contentsOf: consistencyIssues)
        
        if !issues.isEmpty {
            ProductionLogger.warning("Settings validation issues found: \(issues.joined(separator: ", "))")
        }
    ...
    public func getSettingsSummary() -> UnifiedSettingsSummary {
        return UnifiedSettingsSummary(
            performance: UnifiedSettingsSummary.PerformanceSummary(
            ...
            slideshow: UnifiedSettingsSummary.SlideshowSummary(
    ...
    public func applyPresetConfiguration(_ preset: UnifiedPresetConfiguration) {
    ...
    /// Export specific settings types
    public func exportSettings<T: ManagedSettings>(of type: T.Type) throws -> Data {
        switch type {
        case is PerformanceSettings.Type:
            return try performance.exportSettings()
        case is SlideshowSettings.Type:
            return try slideshow.exportSettings()
        case is SortSettings.Type:
            return try sort.exportSettings()
        case is TransitionSettings.Type:
            return try transition.exportSettings()
        case is UIControlSettings.Type:
            return try uiControl.exportSettings()
        default:
            throw UnifiedSettingsError.unsupportedSettingsType
        }
    }
    
    /// Import specific settings types
    public func importSettings<T: ManagedSettings>(of type: T.Type, from data: Data) throws {
        switch type {
        case is PerformanceSettings.Type:
            try performance.importSettings(from: data)
        case is SlideshowSettings.Type:
            try slideshow.importSettings(from: data)
        case is SortSettings.Type:
            try sort.importSettings(from: data)
        case is TransitionSettings.Type:
            try transition.importSettings(from: data)
        case is UIControlSettings.Type:
            try uiControl.importSettings(from: data)
        default:
            throw UnifiedSettingsError.unsupportedSettingsType
        }
    }
    ...
    private func setupSettingsCoordination() {
        ...
        ProductionLogger.debug("Unified settings coordination established")
    }
    
    private func validateImportCompatibility(_ importData: UnifiedSettingsExportData) {
    ...
    private func mergeImportedSettings(_ importData: UnifiedSettingsExportData) throws {
...
/// Data structure for exporting/importing all settings using unified system
public struct UnifiedSettingsExportData: Codable, Sendable {
...
public struct UnifiedSettingsSummary: Sendable {
...
public enum UnifiedPresetConfiguration: String, CaseIterable, Sendable {
...
/// Unified settings specific errors
public enum UnifiedSettingsError: LocalizedError {
    case unsupportedSettingsType
    ...
    public var errorDescription: String? {
        switch self {
        case .unsupportedSettingsType:
            return "Unsupported settings type for operation"
        case .invalidSettingsData:
            return "Invalid settings data format"
        case .importValidationFailed:
            return "Settings import validation failed"
        }
    }
}
...
extension UnifiedAppSettingsCoordinator: @unchecked Sendable {
